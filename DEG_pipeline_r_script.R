### This script produced automatically by the 'rnadeg' automated limma-voom Seqliner analysis pipeline.

zz <- file("/researchers/david.goode/Arcadi_Data/New_Batch/DEG_RESULTS/New_Batch_Design_Matrix_plots/r_script.err", open="wt")
sink(zz)
...
toc <- read.table("/researchers/david.goode/Arcadi_Data/New_Batch/HTSEQ/New_Batch_Design_Matrix_HTSeq_Expression_Matrix.txt", sep="\t", comment="", as.is=T)
...
pdf("/researchers/david.goode/Arcadi_Data/New_Batch/DEG_RESULTS/New_Batch_Design_Matrix_plots/MA_plots_normalisation.pdf", width=14)
...
dev.off()
pdf(file="/researchers/david.goode/Arcadi_Data/New_Batch/DEG_RESULTS/New_Batch_Design_Matrix_plots/MDSplot.pdf")
...
toc <- toc[isexpr, ]
pdf(file="/researchers/david.goode/Arcadi_Data/New_Batch/DEG_RESULTS/New_Batch_Design_Matrix_plots/LIMMA_voom.pdf")
y <- voom(toc, design, plot=TRUE, lib.size=colSums(toc)*norm_factors)
dev.off()

pdf(file="/researchers/david.goode/Arcadi_Data/New_Batch/DEG_RESULTS/New_Batch_Design_Matrix_plots/LIMMA_MDS_plot.pdf")
...
write.table(tab, "/researchers/david.goode/Arcadi_Data/New_Batch/DEG_RESULTS/New_Batch_Design_Matrix_log_dig_matrix.txt", quote=F, sep="\t", row.names=F)

cont <- c("G6-G1", "G7-G2", "G8-G3", "G9-G4", "G10-G5", "G12-G11", "G14-G13", "G16-G15")
...
write.table(tab, "/researchers/david.goode/Arcadi_Data/New_Batch/DEG_RESULTS/New_Batch_Design_Matrix_results.tsv", quote=F, sep="\t", row.names=F)
sink(type="message")
sink()
